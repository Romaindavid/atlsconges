...
export default function FormTemps({ employe }: Props) {
  ...
  function modifierBateau(id: number, valeur: string) {
    setLignesBateaux((prev) =>
      prev.map((l) => (l.id === id ? { ...l, nom_bateau: valeur } : l))
    )
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <section className="bg-white rounded-2xl shadow-sm p-6 border border-marine-100">
          ...
          <p className="text-marine-500 text-sm mb-5">
            Ajoutez une ligne par bateau travaillé aujourd&apos;hui.
          </p>
              ...
              {lignesBateaux.map((ligne) => (
                ...
                >
                  <input
                    ...
                    onChange={(e) => modifierBateau(ligne.id, e.target.value)}
                    placeholder="Nom du bateau"
                    className="flex-1 border-2 border-marine-200 rounded-xl px-4 py-3 text-marine-900 text-base placeholder:text-marine-300 focus:border-orange-500 focus:outline-none transition-colors"
                  />
                  <button
                    type="button"
                    onClick={() => supprimerBateau(ligne.id)}
                    className="p-3 text-danger-600 hover:bg-danger-100 rounded-xl transition-colors flex-shrink-0"
