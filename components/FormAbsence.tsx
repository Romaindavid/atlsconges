...
export default function FormAbsence({ employe }: Props) {
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <section className="bg-white rounded-2xl shadow-sm p-6 border border-marine-100">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div className="sm:col-span-2">
              <label className="block text-marine-700 font-semibold mb-2">Salarié</label>
              <div className="border-2 border-marine-100 bg-marine-50 rounded-xl px-4 py-3 text-marine-800 text-lg font-semibold">
                {nomComplet}
              </div>
            </div>
            ...
            <div>
              ...
              <input
                ...
                type="date"
                value={dateDebut}
                onChange={(e) => { setDateDebut(e.target.value); setJoursManuel(false) }}
                required
            ...
            <div>
              ...
              <input
                ...
                type="date"
                value={dateFin}
                onChange={(e) => { setDateFin(e.target.value); setJoursManuel(false) }}
            ...
            <div className="sm:col-span-2">
              ...
              <div className="flex items-center gap-3">
                <input
                  ...
                  min="1"
                  value={joursOuvres || ''}
                  onChange={(e) => { setJoursManuel(true); setJoursOuvres(parseInt(e.target.value) || 0) }}
                  required
                  placeholder="0"
                ...
                {joursManuel && (
                  <button
                    type="button"
                    onClick={() => { setJoursManuel(false); if (dateDebut && dateFin) setJoursOuvres(calculerJoursOuvres(dateDebut, dateFin)) }}
        ...
        <section className="bg-white rounded-2xl shadow-sm p-6 border border-marine-100">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <div>
              <label className="block text-marine-700 font-semibold mb-2">Date de la demande</label>
              <div className="border-2 border-marine-100 rounded-xl px-4 py-3 text-marine-900 text-lg bg-marine-50">
                {new Date(dateAujourdhui_).toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit', year: 'numeric' })}
              </div>
            </div>
            <div>
              <label className="block text-marine-700 font-semibold mb-2">Salarié</label>
          ...
          <label className="flex items-start gap-4 mt-5 cursor-pointer group">
            ...
            <span className="text-marine-700 text-base group-hover:text-marine-900 transition-colors">
              ...
              <strong className="ml-1 text-marine-800">— {nomComplet}</strong>
            </span>
          </label>
        </section>
